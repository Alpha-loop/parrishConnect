...
import { ActivityIndicator, Button, Checkbox, Chip, Snackbar } from "react-native-paper";
...
const ManageProfile = ({ navigation }) => {
    ...
    const syncAndUpdateData = async () => {
        ...
        try {
            ...
            if (!data.returnObject.status) {
                setDisplaySnack(true)
                setresponseMessage(data.returnObject.statusMessage || 'Ensure all values are entered correctly')
                return;
            }

            if (data.status && data.returnObject.status) {
